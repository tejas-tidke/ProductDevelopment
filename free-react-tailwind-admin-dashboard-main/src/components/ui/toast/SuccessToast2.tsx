...
interface SuccessToast2Props {
  message: string;
  onClose: () => void;
  type?: 'success' | 'error' | 'info' | 'warning';
}

export default function SuccessToast2({ message, onClose, type = 'success' }: SuccessToast2Props) {
  ...
  return (
    <div className="fixed top-4 right-4 z-[9999]">
      <div className={`relative flex w-full max-w-xs items-center rounded-md border px-4 py-3 ${getToastStyles()}`}>
        <span className={`mr-3 flex h-6 w-6 items-center justify-center rounded-full ${getBackgroundClass()}`}>
          {getIcon()}
        </span>
