...
const Toast: React.FC<ToastProps> = ({ message, onClose, type = 'success', position = 'top-right' }) => {
  ...
  return (
    <div className={`fixed ${getPositionClasses()} z-[100000]`}>
      <div className={`relative flex w-full max-w-[460px] items-center rounded-lg border px-5 py-[18px] ${getToastStyles()}`}>
        ...
        <button 
          onClick={onClose}
          className="absolute right-5 top-1/2 -translate-y-1/2 text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200"
        >
          <svg
            width={16}
            height={16}
