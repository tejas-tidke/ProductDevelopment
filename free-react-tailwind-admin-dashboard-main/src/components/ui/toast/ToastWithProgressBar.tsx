...
interface ToastWithProgressBarProps {
  message: string;
  onClose: () => void;
  type?: 'success' | 'error' | 'info' | 'warning';
}

export default function ToastWithProgressBar({ message, onClose, type = 'success' }: ToastWithProgressBarProps) {
  useEffect(() => {
  ...
  return (
    <div className="fixed top-4 right-4 z-[9999]">
      <div className="relative flex w-full max-w-xs items-center overflow-hidden rounded-lg bg-white px-4 py-3 shadow-1 dark:bg-dark-2 dark:shadow-box-dark">
        <span className="absolute bottom-0 left-0 block h-1 w-full bg-stroke dark:bg-dark-3">
          <span className={`absolute left-0 top-0 h-full w-3/4 ${getBgColor()}`}></span>
        </span>
        <span className={`mr-3 flex h-6 w-6 items-center justify-center rounded-full border ${getBorderColor()}`}>
          {getIcon()}
        </span>
        <p className={`text-sm font-medium ${getTextClass()} sm:text-base`}>
          {message}
        </p>
        <button 
          onClick={onClose}
          className="absolute right-3 top-1/2 -translate-y-1/2 text-dark-5 hover:text-primary dark:text-dark-6"
        >
          <svg
            width={14}
            height={14}
